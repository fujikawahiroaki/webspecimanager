...
export const LeafletCoordinateField = ({ record = {}, source }) => {
    var longitude = 0;
    var latitude = 0;
    if (get(record, source)) {
        longitude = get(record, source)["longitude"];
        latitude = get(record, source)["latitude"];
    } else {
        longitude = 0;
        latitude = 0
    };
    ...
    return (
        <MapContainer center={[latitude, longitude]} zoom={15} scrollWheelZoom={true} style={{ height: "50vh" }} >
            <TileLayer
...
LeafletCoordinateField.propTypes = {
...
LeafletCoordinateField.defaultProps = {
    label: 'マップ',
    addLabel: true,
};


export default LeafletCoordinateField;